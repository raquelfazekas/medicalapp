    ...
    return (
      <Card className="shadow-sm border-slate-200">
        <CardContent className="p-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-900 mb-2">Nenhum prontuário encontrado</h3>
          <p className="text-slate-600 mb-4">Clique em &quot;Novo Prontuário&quot; para adicionar o primeiro registro.</p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <div className="space-y-4">
      {documentos.map((record) => (
        <Card key={record.id} className="shadow-sm border-slate-200 hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <h3 className="text-lg font-semibold text-slate-900">{record.type}</h3>
                  <Badge className={getStatusColor("Finalizado")}>{"Finalizado"}</Badge>
                </div>
